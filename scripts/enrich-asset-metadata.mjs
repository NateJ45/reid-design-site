...
function detectTagsFromFilename(filename, folderTags) {
  ...
  if (
    cleaned.startsWith('staci-perkins') &&
...
function generateAltText(filename, detectedTags) {
  ...
  const descriptors = base
    .replace(/^reid-design-/, '')
    .replace(
      /^(living-room|family-room|bedroom|kitchen|bathroom|dining-room|entryway|hallway|office|outdoor|patio|primary-bedroom|master-bedroom|kids-room|exterior)-?/,
      '',
    )
    .replace(/-/g, ', ');
  if (room && descriptors) {
...
console.log(
  `Plan: enrich ${enrichments.length} assets, skip ${skipped} (already have metadata; pass --force to overwrite).`,
);
...
if (writeClient) {
  ...
  for (const e of enrichments) {
    try {
      ...
      failedPatches.push({
        id: e.asset._id,
        filename: e.asset.originalFilename,
        error: err.message,
      });
    }
  }
...
if (!writeClient || patchFailed > 0) {
  ...
  if (toFallback.length > 0) {
    ...
    console.log(
      `  Run: cd studio && npx sanity dataset import "${assetPath}" production --replace`,
    );
  }
}
...
if (failedPatches.length > 0 && failedPatches.length <= 5) {
  ...
  failedPatches
    .slice(0, 5)
    .forEach((f) => console.log(`  ${f.filename}: ${f.error.slice(0, 100)}`));
}
