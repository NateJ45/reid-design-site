...
- **Home-base locality** — `businessInfo.city` / `state` / `serviceRegion` (one source of truth, centralized 2026-06-11). Drives the footer location line ("{city}, {state}" / "Serving {serviceRegion}") AND the LocalBusiness `addressLocality` / `addressRegion` plus the per-service `areaServed`, all via `getSiteSettings` with stable `Plainfield` / `IN` / `Greater Indianapolis` code fallbacks. NAP-critical: the field descriptions warn it must match the Google Business Profile. The only locality reference that stays static is the print-stylesheet line (CSS can't read Sanity).
...
- **Style quiz** — `styleQuiz` singleton (SEO title/description/share image, intro copy, questions + image answers + archetype weights, optional qualifiers, archetypes with images + recommended-service reference, email gate mode/copy, result routing). Powers `/quiz`. SEO title/description fall back to the built-in defaults in `quiz.astro` when left blank.
- **Budget calculator** — `budgetCalculator` singleton (SEO title/description/share image, intro copy, rooms, scope options, add-ons, result copy with `{{low}}`/`{{high}}` placeholders, disclaimer, CTA label, consult-price note). Powers `/calculator`. SEO title/description fall back to the built-in defaults in `calculator.astro` when left blank.
...
- **Custom pages Staci builds herself** — the `page` doc type (page builder). Staci creates a brand-new page from the section library (hero, rich text, image + text, gallery, quote, stats, CTA band, video, spacer), picks a slug, and optionally adds it to the nav/footer. Reserved route slugs are blocked. Renders via `SectionRenderer`. See [Page builder](page-architecture.md) for the full block list.
- **Extra sections on the standard pages** — every standard page singleton that isn't a free-form page builder still has an optional **"Extra sections"** group (`additionalSections`, same library as the page builder). It appends library blocks to the bottom of the page, above the final CTA where one exists, without touching the page's built-in structure. Wired on `homePage`, `aboutPage`, `servicesPage`, `processPage`, `resourcesPage`, `pressPage`, `eDesignPage`, `giftPage` (these use `pageBuilder` markers, so "extra" blocks are added by inserting a General block between markers) **and** on `faqPage`, `contactPage`, `privacyPage`, `journalPage`, `portfolioPage` via a dedicated `additionalSections` array rendered by a second `SectionRenderer`. Leaving the array empty changes nothing. This is the "let Staci extend any page" escape hatch.
...
- **Footer column heading + link labels** — the five footer column headings ("Studio", "Work", "Free tools & guides", "Latest projects", "Get in touch") and the short link labels under them are structural navigation scaffolding in `Footer.astro`, deliberately not editorial. The _destinations_ are already governed by section-visibility toggles and the page singletons; the labels are fixed so the footer's information architecture stays coherent. Surfacing five label fields in Site Settings would clutter the editor for an amateur with near-zero benefit. (Audited 2026-06-10.)
- **Print-stylesheet footer line** — `globals.css` prints `"Reid Design LLC · reiddesignllc.com · Plainfield, IN"` via a `@media print` pseudo-element. CSS can't read Sanity, so this one locality reference stays static (print-only, near-zero visibility). Everything else NAP-related is now editable (see the locality note below).
- **Empty-state / coming-soon fallback copy** — the "Case studies are on the way" (portfolio), "The first journal entries are on their way" (journal), "FAQ content is being updated" (faq), and "Coming soon" (quiz/calculator/shop) messages are graceful-degradation fallbacks that render only while the corresponding content is absent. Once Staci adds a project / post / question, the fallback disappears. They are intentionally hardcoded because they are not content Staci would ever hunt down a field to edit; she "changes" them by adding the real content. (Audited 2026-06-10.)
