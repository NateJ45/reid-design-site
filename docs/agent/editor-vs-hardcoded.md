...
- **Site-wide locality ("Plainfield, IN" / "Greater Indianapolis")** — appears in the `Footer.astro` brand block, the `/contact` sidebar, the LocalBusiness JSON-LD (`src/lib/schemas.ts` `addressLocality`/`areaServed`), and several page meta strings. It is a stable business constant, not per-page copy. Making *only* the footer line editable would desync it from the structured data and the rest of the site, so it stays hardcoded. If the studio ever relocates or rebrands its service region, the right move is a **planned session** that centralizes locality into one source (e.g. `site.ts` or `businessInfo`) and threads it through every consumer at once, then re-validates the JSON-LD against Google Rich Results. Do not do this piecemeal. (Audited 2026-06-10.)
