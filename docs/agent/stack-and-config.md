...
- Astro **6.3.8** with TypeScript in strict mode and `output: 'static'`. Requires Node 22.12+. (Pinned: a 2026-06-11 bump to 6.4.6 + `@astrojs/cloudflare` 13.7.0 was reverted. Under `imageService: 'compile'`, those versions stopped emitting the original for any local image used only via `getImage()` — the theme-swap header/footer logos, which never render their original PNG — so the build-time optimizer hit `ENOENT` reading `dist/_astro/logo-*.png` and every Cloudflare build failed. Do not re-bump past 6.3.8 until that upstream regression is fixed, and verify any retry with a cleared `node_modules/.astro` cache, because a warm cache hides it.)
