 ...
 * learned the hard way in a deployed Studio. The overlay will happily OUTLINE
 * an element whose attribute names a bare array item, which is what gives the
 * section its insert / duplicate / drag menu. But a custom overlay COMPONENT
 * only mounts on a node the Studio schema resolves to a FIELD, and an array
 * item on its own is not one: `getField` returns nothing and the component
 * resolver is never called. Pointing a small handle at
 * `...[_key=="..."].width` gives the same section a node that DOES resolve,
 * which is how the layout card gets on screen. See
 * src/components/preview/overlay/index.ts.
 */
