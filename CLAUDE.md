...
- **Sanity 6.9.1, one package, Studio embedded at `/studio`.** Schemas live in `src/sanity/schemaTypes/`; the Studio config is the repo-root `sanity.config.ts`. The nested `studio/` package is GONE (folded in 2026-08-28) and so is the separately deployed `reid-design.sanity.studio`. `npm run typegen` runs from the root. The Sanity dependency set is pinned to a combination known to work TOGETHER: `sanity` 6.9.1, `@sanity/vision` 6.9.1, `@sanity/ui` 3.5.4, `styled-components` 6.4.3, `@sanity/client` 7.26.2, `@sanity/visual-editing` 5.7.3, `@sanity/preview-url-secret` 4.1.5, `react`/`react-dom`/`react-is` all exactly 19.2.7, plus `overrides` for `sanity-plugin-utils` and `@sanity/visual-editing` (moved together on 2026-09-06). Bumping one in isolation breaks the Studio at browser runtime, not at build time.
